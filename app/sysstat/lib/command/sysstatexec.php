...
class sysstat_command_sysstatexec extends base_shell_prototype{
    ...
    var $command_execManager = '根据条件执行运营商统计';
    var $command_execManager_options = array(
        'timeStart'=>array('title'=>'具体某个日期，如：2015-06-01'),
        'timeEnd'=>array('title'=>'具体某个日期，如：2015-06-02'),
    );
    function command_execManager($timeStart,$timeEnd)
    {
        $nowTime = strtotime(date('Y-m-d'.'00:00:00',time()));
        if(strtotime($timeStart)>strtotime($timeEnd)||strtotime($timeStart)>$nowTime)
        {
        ...
        else
        {
            if($timeStart&&$timeEnd)
            {
                ...
                { 
                    ...
                    if(kernel::single('sysstat_tasks_operatorday')->exec($params))
                    {
                        return true;
                    }
                }
            ...
            else
            {
                ...
                if(kernel::single('ectools_tasks_statistic_day')->exec($params))
                {
                    return true;
                }
            }
    ...
    var $command_execallManager = '强制重新计算运营商所有的订单数据到统计表中';
    function command_execallManager()
    {
        $timeEnd = strtotime(date('Y-m-d 23:59:59', strtotime('-1 day')));
        $timeStart = strtotime('2015-01-01 00:00:00');
        $count = ceil(($timeEnd-$timeStart)/(3600*24));
    ...
    var $command_execOldManager = '根据条件执行运营商统计(老)';
    var $command_execOldManager_options = array(
        'timeStart'=>array('title'=>'具体某个日期，如：2015-06-01'),
        'timeEnd'=>array('title'=>'具体某个日期，如：2015-06-02'),
    );
    function command_execOldManager($timeStart,$timeEnd)
    {
        $nowTime = strtotime(date('Y-m-d'.'00:00:00',time()));
        if(strtotime($timeStart)>strtotime($timeEnd)||strtotime($timeStart)>$nowTime)
        {
    ...
    var $command_exeOldcallManager = '强制重新计算运营商所有的订单数据到统计表中(老)';
    function command_execOldallManager()
    {
        $timeEnd = strtotime(date('Y-m-d 23:59:59', strtotime('-1 day')));
        $timeStart = strtotime('2015-01-01 00:00:00');
        $count = ceil(($timeEnd-$timeStart)/(3600*24));
