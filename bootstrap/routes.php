...
|--------------------------------------------------------------------------
| api
|-------------------------    nkbjjjfasfdasklnf m;'dl,gadgfdg;-------------------------------------------------
 */
route::group(array('prefix'=>'notebookHu'),function(){
	route::get('default.html',['uses'=>'notebookHu_ctl_default@index']);
	route::get('default.php',['uses'=>'notebook_ctl_default@desktop']);
});

route::pattern('id','[0-9]');
route::get('userid/{id}',function ($id){
	return 'this is a number'.$id.'';
});
route::get('user/profile',array('as'=>'profile','uses'=>'notebookHu_ctl_default@index'));
route::get('/test',function()
{
	return 'this is a test';
});
route::get('/test1/{test?}',function ($name='this is a name')
{
	return '开始打印'.$name.'';
});
route::get('foo/test',function() {
	return 'this a foo/test';
});
route::any('foo',function (){
	return 'hello world';
});
route::get('httpstest',array('https',function(){
	return 'this Must be https';
}));
...
route::get(array('prefix'=>array('test'),array('as'=>'profile','uses'=>'test')));
...
route::get('username/{name?}',function ($name = 'king'){
	return $name;
...
route::group(array('middleware' => ['topc_middleware_redirectIfFromWap', 'theme_middleware_preview']), function() {
    ...
    */
    ...
    route::group(array('prefix'=>'topc'),function (){
    	
    	route::get('bookview.html',['uses'=>'topc_ctl_booktop@index']);
    	route::post('bookview.html',['uses'=>'notebookHu_ctl_booktop@addnew']);
